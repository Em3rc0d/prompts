...
1. `STATUS_CURRENT.md`
2. `STARTER_CODE_REVIEW_G14_TEST_WEBHOOK_CREATED_2026-09-08.json`
3. `STARTER_CODE_REVIEW_G14_TEST_PRODUCT_PUBLISHED_2026-09-08.json`
4. `STARTER_CODE_REVIEW_RELEASE_STATE_G14_PROVIDER_HANDOFF_2026-09-08.json`
5. `STARTER_CODE_REVIEW_EDITION_RELEASE_PROFILE_RC2_V1.json`
6. `STARTER_CODE_REVIEW_G12_PACK_REBUILD_RC2_PASS_2026-09-08.json`
7. `STARTER_CODE_REVIEW_G13_PACK_QA_RC2_PASS_2026-09-08.json`
8. `STARTER_CODE_REVIEW_G14_LEMONSQUEEZY_HANDOFF_V1.md`
...
A Test-mode `order_created` webhook also now exists:
...
Test webhook object          PASS
Test-mode byte custody       NOT OBSERVABLE BY PROVIDER DESIGN
Vercel Test apply            NOT YET OBSERVED
test checkout/order          NOT YET OBSERVED
signed order_created         NOT YET OBSERVED
Live byte custody            NOT YET OBSERVED
```

Lemon Squeezy disables file downloads in Test mode, so an HTTP 403 from a Test-mode file download is not treated as a corrupted artifact or custody failure. Byte custody is reserved for a controlled later Live canary.
...
- `tools/pm_g14_lemonsqueezy_probe.py` performs read-only Test-mode discovery and metadata validation.
- `tools/pm_g14_lemonsqueezy_test_setup.py` created exactly one Test-mode `order_created` webhook and wrote an owner-only Vercel handoff file.
- `tools/pm_g14_vercel_test_apply.py` imports that handoff into `prompt-quarry-stage`, redeploys, verifies the private checkout gate, and emits only the final Lemon Test checkout URL.
...
The route is present and POST-only. Public-sale defaults remain fail closed:
...
```text
Vercel Test env import                     NOT YET OBSERVED
Vercel Test redeploy                       NOT YET OBSERVED
private provider-test checkout redirect    NOT YET OBSERVED
