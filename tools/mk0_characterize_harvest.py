...
def read_body(r):
    ref=r.get("provenance",{}).get("raw_record_ref"); p=Path(ref) if ref else None
...
def conf(best,second,base=.79,ceiling=.98):
    if best<=0:return .72
    return min(ceiling,base+min(best,6)*.025+max(best-second,0)*.015)

def characterize(r,body,seen,policy):
    text=f"{r.get('title') or ''}\n{body}"; domain,db,ds=score_rules(text,DOMAIN_RULES); intent,ib,is_=score_rules(text,INTENT_RULES)
    techniques=detected(text,TECHNIQUE_RULES); architecture=detected(text,ARCH_RULES); fp=fingerprint(body or r["canonical_url"]); duplicate=seen.get(fp)
...
def persist(records,output,receipt):
    output.parent.mkdir(parents=True,exist_ok=True); output.write_text("".join(json.dumps(r,ensure_ascii=False)+"\n" for r in records),encoding="utf-8")
    routes=Counter(r["route"] for r in records); domains=Counter(r["classification"]["domain"] for r in records); fam=Counter(r["classification"]["family"] for r in records); tech=Counter(t for r in records for t in r["techniques"]); ag=[r["confidence"]["aggregate"] for r in records]
    research=sum(r["eligibility"]["golden_research_eligibility"]["eligible"] for r in records); distribution=sum(r["eligibility"]["distribution_eligibility"]["eligible"] for r in records)
    bands={">=0.95":sum(x>=.95 for x in ag),"0.90-<0.95":sum(.90<=x<.95 for x in ag),"<0.90":sum(x<.90 for x in ag)}
    payload={"schema":"prompt-quarry-characterization-receipt-v1","batch_id":"mk0-characterization-001","status":"PASS","created_at":utc_now(),"records":len(records),"routes":dict(routes),"confidence_bands":bands,"eligibility":{"golden_research_eligible":research,"distribution_eligible":distribution},"domains":dict(domains),"top_families":dict(fam.most_common(12)),"top_techniques":dict(tech.most_common(12)),"aggregate_confidence":{"min":min(ag),"max":max(ag),"mean":round(sum(ag)/len(ag),4)},"duplicates":sum(1 for r in records if r.get("duplicate_of")),"claim_boundary":"Heuristic MK0 characterization only. Research eligibility is independent from redistribution permission. UNKNOWN license remains UNKNOWN and blocks distribution, not internal Golden candidacy."}
    receipt.parent.mkdir(parents=True,exist_ok=True); receipt.write_text(json.dumps(payload,indent=2)+"\n")

def main():
    p=argparse.ArgumentParser(); p.add_argument("--input",type=Path,default=SOURCE_RECORDS); p.add_argument("--output",type=Path,default=QUEUE_ROOT/"batch-001.jsonl"); p.add_argument("--receipt",type=Path,default=Path("mk0/analysis/harvester/characterization-batch-001-receipt.json")); a=p.parse_args(); policy=load_policy(); seen={}; records=[characterize(r,read_body(r),seen,policy) for r in load_jsonl(a.input)]; validate(records); persist(records,a.output,a.receipt); print(json.dumps({"records":len(records),"output":str(a.output),"receipt":str(a.receipt)},indent=2))
