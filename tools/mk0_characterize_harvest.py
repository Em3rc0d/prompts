...
SOURCE_RECORDS=Path("mk0/normalized/harvester/source-records.jsonl"); CANDIDATE_SCHEMA=Path("mk0/harvester/CANDIDATE_RECORD.schema.json"); QUEUE_ROOT=Path("mk0/golden-dataset/candidate-queue"); REFERENCE_ROOT=Path("mk0/reference-corpus"); EXTRACTION_ROOT=Path("mk0/extraction-queue")
...
def persist(records,output,receipt,reference_output,extraction_output,policy):
 ...
 containers=[r for r in records if r["semantic_gate"]["disposition"]=="EXTRACTION_REQUIRED"]; extraction_output.parent.mkdir(parents=True,exist_ok=True); extraction_output.write_text("".join(json.dumps(r,ensure_ascii=False)+"\n" for r in containers),encoding="utf-8")
 ...
 payload={"schema":"prompt-quarry-characterization-receipt-v1","batch_id":"mk0-characterization-001","policy_version":policy["policy_version"],"status":"PASS","created_at":utc_now(),"records":len(records),"routes":dict(routes),"semantic_artifact_classes":dict(semantics),"semantic_dispositions":dict(dispositions),"reference_corpus_records":len(refs),"prompt_container_records":len(containers),"confidence_bands":bands,"eligibility":{"golden_research_eligible":research,"distribution_eligible":distribution},"domains":dict(domains),"top_families":dict(fam.most_common(12)),"top_techniques":dict(tech.most_common(12)),"aggregate_confidence":{"min":min(ag),"max":max(ag),"mean":round(sum(ag)/len(ag),4)},"duplicates":sum(1 for r in records if r.get("duplicate_of")),"claim_boundary":"Semantic artifact identity is gated before Golden routing. REFERENCE_CORPUS is non-canonical reference material, not truth and not Golden evidence. PROMPT_CONTAINER sources require governed child extraction and are not prompts themselves. UNKNOWN license remains UNKNOWN and blocks distribution. Human approval does not establish behavioral certification."}; receipt.parent.mkdir(parents=True,exist_ok=True); receipt.write_text(json.dumps(payload,indent=2)+"\n")
def main():
 p=argparse.ArgumentParser(); p.add_argument("--input",type=Path,default=SOURCE_RECORDS); p.add_argument("--output",type=Path,default=QUEUE_ROOT/"batch-001.jsonl"); p.add_argument("--reference-output",type=Path,default=REFERENCE_ROOT/"batch-001.jsonl"); p.add_argument("--extraction-output",type=Path,default=EXTRACTION_ROOT/"batch-001.jsonl"); p.add_argument("--receipt",type=Path,default=Path("mk0/analysis/harvester/characterization-batch-001-receipt.json")); a=p.parse_args(); policy=load_policy(); seen={}; records=[characterize(r,read_body(r),seen,policy) for r in load_jsonl(a.input)]; validate(records); persist(records,a.output,a.receipt,a.reference_output,a.extraction_output,policy); print(json.dumps({"records":len(records),"policy_version":policy["policy_version"],"output":str(a.output),"reference_output":str(a.reference_output),"extraction_output":str(a.extraction_output),"receipt":str(a.receipt)},indent=2))
