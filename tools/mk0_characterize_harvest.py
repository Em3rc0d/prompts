...
SOURCE_RECORDS=Path("mk0/normalized/harvester/source-records.jsonl"); CANDIDATE_SCHEMA=Path("mk0/harvester/CANDIDATE_RECORD.schema.json"); QUEUE_ROOT=Path("mk0/golden-dataset/candidate-queue")
...
def characterize(r,body,seen,policy):
 text=f"{r.get('title') or ''}\n{body}"; domain,db,ds=score_rules(text,DOMAIN_RULES); intent,ib,is_=score_rules(text,INTENT_RULES); techniques=detected(text,TECHNIQUE_RULES); architecture=detected(text,ARCH_RULES); fp=fingerprint(body or r["canonical_url"]); duplicate=seen.get(fp)
