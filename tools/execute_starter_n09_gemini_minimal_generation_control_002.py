#!/usr/bin/env python3
"""Governed one-shot minimal Gemini generateContent diagnostic v2.

This is NOT a G05 workflow execution. It sends only the fixed prompt
"Return exactly: OK" to gemini-3.5-flash with thinkingLevel=minimal and
maxOutputTokens=64. Exactly one provider request maximum, zero automatic
retries, local evidence only.
"""
...
EXACT_AUTH = "AUTORIZO PM-GEMINI-MINIMAL-GEN-CTRL-002: 1 ejecución, 0 reintentos."
...
def finish_reasons(payload: dict) -> list[str | None]:
    return [candidate.get("finishReason") for candidate in payload.get("candidates") or []]
...
def main() -> int:
    ...
    checks = {
        "control_id": plan.get("control_id") == CONTROL_ID,
        "state": plan.get("state") == "PREPARED_ZERO_MODEL_PREFLIGHT_REQUIRED",
        "model": plan.get("model") == MODEL,
        "prompt": plan.get("prompt") == PROMPT,
        "max_output_tokens": config.get("maxOutputTokens") == 64,
        "thinking_minimal": thinking.get("thinkingLevel") == "minimal",
        "single_request": plan.get("maximum_provider_requests") == 1,
        "zero_retries": plan.get("automatic_retries") == 0,
        "not_g05": plan.get("g05_claim") == "NONE",
        "workflow_not_sent": plan.get("workflow_candidate_sent") is False,
        "behavioral_observation_forbidden": plan.get("behavioral_observation_allowed") is False,
    }
    bad = [name for name, ok in checks.items() if not ok]
    if bad:
        raise SystemExit("blocked: control contract drift: " + ", ".join(bad))
    ...
    try:
        ...
        receipt = {
            "schema": "prompt-machine-starter-n09-gemini-minimal-generation-control-result-v2",
    ...
    try:
        ...
        reasons = finish_reasons(payload)
    ...
    exact_ok = text.strip() == "OK"
    max_tokens = "MAX_TOKENS" in reasons
    if status == 200 and exact_ok and not max_tokens:
        classification = "MINIMAL_GENERATION_CONTROL_PASS_PATH_AND_VISIBLE_TEXT"
        control_pass = True
    elif status == 200 and not text:
        classification = "GENERATION_PATH_AVAILABLE_BUT_VISIBLE_OUTPUT_UNOBSERVED_REVIEW_RAW_RESPONSE"
        control_pass = False
    elif status == 200:
        classification = "GENERATION_PATH_AVAILABLE_SEMANTIC_CONTROL_MISMATCH"
        control_pass = False
    else:
        classification = "MINIMAL_GENERATION_UNEXPECTED_SUCCESS_STATUS"
        control_pass = False
    ...
    receipt = {
        ...
        "response_text_exact_ok": exact_ok,
    ...
    write_json(out / "receipt.json", receipt)
    print(json.dumps(receipt, indent=2, sort_keys=True))
    return 0 if control_pass else 3
