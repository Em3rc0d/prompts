#!/usr/bin/env python3
"""Prompt Machine Starter N09 — governed local WSL Gemini G05 retest 005.

This retest preserves the frozen Starter candidate, exact 8100-byte envelope,
gemini-3.5-flash model, and request configuration used by G05-FALLBACK-004.
It is justified only after PM-GEMINI-MINIMAL-GEN-CTRL-002 observed HTTP 200
with visible exact OK output. Exactly one provider request maximum, zero retries,
local evidence only, and mandatory human review.
"""
...
def load_contracts() -> tuple[bytes, dict, dict, dict, dict, dict]:
    ...
    checks = {
        ...
        "retest_scope": plan.get("experiment_class") == "GOVERNED_PROVIDER_RECOVERY_RETEST",
        "provider_recovery_control": control.get("control_id") == "PM-GEMINI-MINIMAL-GEN-CTRL-002" and control.get("control_pass") is True and control.get("http_status") == 200,
        "same_prompt": plan.get("candidate_prompt_mutated") is False,
        "same_request_config": plan.get("request_configuration_matches_g05_fallback_004") is True and plan.get("generation_config", {}).get("maxOutputTokens") == EXPECTED_MAX_OUTPUT_TOKENS,
...
def main() -> int:
    ...
    if args.model != EXPECTED_MODEL or not MODEL_RE.fullmatch(args.model):
        raise SystemExit("blocked: model differs from frozen retest plan")
    ...
    if out != planned:
        raise SystemExit("blocked: output directory differs from frozen retest plan")
    if out.exists():
    ...
    try:
        ...
        raw = exc.read()
        receipt = {
            "schema": "prompt-machine-starter-n09-gemini-wsl-retest-v5-failure",
        ...
        receipt = {
            "schema": "prompt-machine-starter-n09-gemini-wsl-retest-v5-transport-failure",
            "attempt_id": ATTEMPT_ID,
            "execution_id": execution_id,
            "case_id": CASE_ID,
            "provider_requests_attempted": 1,
    ...
    try:
        ...
        receipt = {
            "schema": "prompt-machine-starter-n09-gemini-wsl-retest-v5-invalid-response",
            "attempt_id": ATTEMPT_ID,
            "execution_id": execution_id,
            "provider_requests_attempted": 1,
            "automatic_retries": 0,
    ...
    if not output:
        receipt = {
            "schema": "prompt-machine-starter-n09-gemini-wsl-retest-v5-empty-output",
            "attempt_id": ATTEMPT_ID,
            "execution_id": execution_id,
            "provider_requests_attempted": 1,
            "automatic_retries": 0,
    ...
    evidence = {
        "schema": "prompt-machine-starter-n09-gemini-wsl-retest-observation-v1",
        "version": "1.0.0",
        "attempt_id": ATTEMPT_ID,
        "execution_id": execution_id,
        "case_id": CASE_ID,
        ...
        "model_requested": args.model,
        "model_scope": "gemini-3.5-flash",
        "same_model_scope_as_attempt_004": True,
        ...
        "state": "CLEAN_LOCAL_WSL_GEMINI_RETEST_RUNTIME_OBSERVED_HUMAN_REVIEW_REQUIRED",
    ...
    write_json(out / "review-packet.json", {
        "schema": "prompt-machine-starter-n09-gemini-wsl-retest-review-packet-v1",
        "attempt_id": ATTEMPT_ID,
        "execution_id": execution_id,
        "case_id": CASE_ID,
