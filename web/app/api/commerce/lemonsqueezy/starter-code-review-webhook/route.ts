...
import { STARTER_CODE_REVIEW_RELEASE, type CommerceGate } from "@/lib/starter-code-review-release";
...
function loadConfig(): LemonSqueezyConfig | null {
  const commerceMode = currentStarterCodeReviewCommerceMode();
  ...
  const webhookSecret = process.env.LEMONSQUEEZY_STARTER_CODE_REVIEW_WEBHOOK_SECRET;
  const storeId = process.env.LEMONSQUEEZY_STORE_ID;
  const productId = process.env.LEMONSQUEEZY_STARTER_CODE_REVIEW_PRODUCT_ID;
  const variantId = process.env.LEMONSQUEEZY_STARTER_CODE_REVIEW_VARIANT_ID;
  ...
  return {
    ...
    commerceMode,
    commerceGate,
    release: STARTER_CODE_REVIEW_RELEASE,
  };
}
...
export async function POST(request: Request) {
  ...
  if (!config) {
    return Response.json(
      { ok: false, error: "starter_code_review_commerce_not_configured" },
      { status: 503 },
    );
  }
  ...
  console.info("PM_STARTER_CODE_REVIEW_COMMERCE_EVENT", JSON.stringify(evaluation.evidence));
