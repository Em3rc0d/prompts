...
export default function WorkflowGuidePage() {
  return <main>
    <section className="pageHero"><div className="wrap">
      <div className="eyebrow">VERLUNE / WORKFLOW DESIGN</div>
    ...
    <section className="section"><div className="wrap"><div className="grid2">
      <article className="card"><h3>The random-prompt loop</h3><p>You open a blank chat, explain the task from memory, notice the answer missed something, add another instruction, repair the format, and repeat the same setup next time.</p><p>The problem is not natural-language prompting itself. The problem is that the operating knowledge stays trapped inside one conversation.</p></article>
    ...
    <section className="section"><div className="wrap">
      ...
      <div className="pipeline">
        ...
        <article className="pipelineStep"><div className="pipelineTop"><span>02</span><code>PROCESS</code></div><h3>Repeatable steps</h3><p>Make the important operating sequence stable enough that another run does not depend on remembering yesterday's chat.</p></article>
    ...
    <section className="cta"><div className="wrap"><h2>Try the idea on a real task.</h2><p>The current Free Library has nineteen complete assets across eight categories: sixteen prompts and three workflows. Use one on a real task before deciding whether this approach is useful to you.</p><div className="actions"><Link className="btn btnPrimary" href="/free">Explore the Free Library →</Link><Link className="btn btnSecondary" href="/learn">Back to Learn</Link></div></div></section>
  </main>;
}
