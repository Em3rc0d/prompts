...
function buildArchive(){return gunzipSync(Buffer.from(PAYLOAD,"base64"));}
...
export async function GET(request){
  ...
  if(archive.length!==EXPECTED_SIZE||sha!==EXPECTED_SHA){console.error("PQ_FREE_INTEGRITY_FAILURE",JSON.stringify({size:archive.length,sha}));return Response.json({ok:false,error:"free_pack_integrity_failure",expected_size:EXPECTED_SIZE,observed_size:archive.length,expected_sha256:EXPECTED_SHA,observed_sha256:sha},{status:500});}
