...
Status: `SUCCESSOR CANDIDATE / G07 IMPLEMENTED / STATIC AUDIT REQUIRED / BEHAVIOR NOT YET RETESTED`
...
This successor preserves the v1 authority/data boundary and adds explicit calibration rules for negative evidence, unobserved external controls, and conditional downstream impact.
...
- `LIKELY` — strong inference, but at least one material fact or control boundary remains unobserved.
...
Dismiss or downgrade a candidate when it is only style preference, lacks an explainable failure mechanism, is disproved by supplied context, depends on an undefended assumption, duplicates a stronger finding, or has no material consequence.
